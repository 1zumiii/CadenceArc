...
namespace CadenceArc::Tests
{
	...
	static FString GraphValidationSnapshot(const UCadenceArcGraph* Graph)
	{
		FString Snapshot = FString::Printf(TEXT("Entry=%s;Age=%.17g;Nodes=%d;"),
			*Graph->EntryActionTag.ToString(), Graph->MaxBufferedInputAgeSeconds, Graph->Nodes.Num());
...
{
	using namespace CadenceArc::Tests;
	struct FCase { const TCHAR* Name; TFunction<void(UCadenceArcGraph*)> Configure; FString Fragment; };
	const FCase Cases[] = {
		{TEXT("Empty nodes"), [](UCadenceArcGraph* Graph) { Graph->Nodes.Reset(); }, TEXT("Nodes")},
		{TEXT("Invalid node tag"), [](UCadenceArcGraph* Graph) { Graph->Nodes[0].ActionTag = FGameplayTag::EmptyTag; }, TEXT("index 0")},
		{TEXT("Duplicate node tag"), [](UCadenceArcGraph* Graph) { AddNode(Graph, Action_Root); }, Action_Root.GetTag().ToString()},
		{TEXT("Invalid entry"), [](UCadenceArcGraph* Graph) { Graph->EntryActionTag = FGameplayTag::EmptyTag; }, TEXT("EntryActionTag")},
		{TEXT("Missing entry"), [](UCadenceArcGraph* Graph) { Graph->EntryActionTag = Input_Light; }, Input_Light.GetTag().ToString()},
	};
	for (const FCase& Case : Cases)
	{
		UCadenceArcGraph* Graph = MakeValidGraph();
...
bool FCadenceArcGraphValidationAgeTest::RunTest(const FString& Parameters)
{
	...
	struct FCase { double Age; bool bValid; };
	const FCase Cases[] = {{0.0, true}, {0.25, true}, {-0.25, false}, {NaN, false}, {Infinity, false}, {-Infinity, false}};
	for (const FCase& Case : Cases)
	{
		UCadenceArcGraph* Graph = MakeValidGraph();
...
bool FCadenceArcGraphValidationTransitionTest::RunTest(const FString& Parameters)
{
	...
	struct FCase { const TCHAR* Name; TFunction<void(UCadenceArcGraph*)> Configure; FString Fragment; };
	const FCase Cases[] = {
		{TEXT("Invalid input"), [](UCadenceArcGraph* Graph) { Graph->Nodes[0].Transitions[0].InputTag = FGameplayTag::EmptyTag; }, TEXT("index 0")},
		{TEXT("Invalid target"), [](UCadenceArcGraph* Graph) { Graph->Nodes[0].Transitions[0].TargetActionTag = FGameplayTag::EmptyTag; }, TEXT("index 0")},
		{TEXT("Missing target"), [](UCadenceArcGraph* Graph) { Graph->Nodes[0].Transitions[0].TargetActionTag = Input_Light; }, Input_Light.GetTag().ToString()},
	};
	for (const FCase& Case : Cases)
	{
		UCadenceArcGraph* Graph = MakeValidGraph();
...
bool FCadenceArcGraphValidationDuplicateInputTest::RunTest(const FString& Parameters)
{
	...
	for (const FGameplayTag& Target : Targets)
	{
		...
		TestEqual(*FString::Printf(TEXT("Duplicate input to %s is invalid"), *Target.ToString()),
			static_cast<uint8>(ValidateGraph(Graph, Messages)), static_cast<uint8>(EDataValidationResult::Invalid));
		TestTrue(TEXT("Duplicate diagnostic identifies source and input"),
			HasValidationMessage(Messages, Action_Root.GetTag().ToString()) && HasValidationMessage(Messages, Input_Light.GetTag().ToString()));
	}
	return !HasAnyErrors();
}
